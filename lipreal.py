...
class LipReal(BaseReal):
    ...
    def paste_back_frame(self,pred_frame,idx:int):
        ...
        # Validate and clamp bbox coordinates to frame bounds
        ...
        if x2 <= x1 or y2 <= y1:
            logger.warning(f"Invalid bbox at idx {idx} after clamping: x1={x1}, x2={x2}, y1={y1}, y2={y2}, frame_shape={combine_frame.shape}, original_bbox={bbox}. Returning original frame.")
        ...
        # Validate target size is positive
        if target_w <= 0 or target_h <= 0:
            logger.warning(f"Invalid target size at idx {idx}: target_w={target_w}, target_h={target_h}, bbox=({y1},{y2},{x1},{x2}). Returning original frame.")
            return combine_frame
        
        # Validate pred_frame dimensions
        if pred_frame is None or pred_frame.size == 0:
            logger.warning(f"Empty pred_frame at idx {idx}. Returning original frame.")
            return combine_frame
        
        try:
            # Resize predicted frame to match target region
            res_frame = cv2.resize(pred_frame.astype(np.uint8), (target_w, target_h))
            # Validate resized frame dimensions match target
            if res_frame.shape[:2] != (target_h, target_w):
                logger.warning(f"Resized frame shape mismatch at idx {idx}: expected ({target_h},{target_w}), got {res_frame.shape[:2]}. Returning original frame.")
                return combine_frame
            # Paste resized frame back into original frame
            combine_frame[y1:y2, x1:x2] = res_frame
        except Exception as e:
            logger.warning(f"paste_back_frame error: {e}. bbox=({y1},{y2},{x1},{x2}), target_size=({target_w},{target_h}), frame_shape={combine_frame.shape}, pred_shape={pred_frame.shape if pred_frame is not None else None}, original_bbox={bbox}")
            return combine_frame
